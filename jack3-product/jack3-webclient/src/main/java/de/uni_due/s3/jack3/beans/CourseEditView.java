...
import de.uni_due.s3.jack3.business.*;
...
@ViewScoped
@Named
public class CourseEditView extends AbstractView implements Serializable {
    ...
    @Inject
    private CompetenceGoalBusiness competenceGoalBusiness;
    ...
    private List<CompetenceGoal> competenceGoals = new ArrayList<>();
    ...
    public void loadCourse() throws IOException {
        try {
            if (isFrozen()) {
                ...
                revisionCount = courseBusiness.getNumberOfRevisions(course);
                competenceGoals = competenceGoalBusiness.getCompetenceGoalsByCourse(course);
                loadAvailableFrozenCourses();
            }
    ...
    public CompetenceGoalBusiness getCompetenceGoalBusiness() {
        return competenceGoalBusiness;
    }

    public List<Competence> getCompetenciesNotAllocated() {
        return competenceGoalBusiness.getAllCompetenciesOfCourseSubjectNotAllocatedAsGoal(course);
    }

    public void addCompetenceGoal(Competence competence) throws CompetenceGoalException {
        competenceGoalBusiness.addNewCompetenceGoalToCourse(
                new CompetenceGoal(competence, (Course) course, 50, 1));
        refreshCompetenceGoals();
    }

    public void updateCompetenceGoal(CompetenceGoal competenceGoal) throws CompetenceGoalException {
        if (competenceGoals.contains(competenceGoal)) {
            if(competenceGoalBusiness.updateCompetenceGoal(competenceGoal)){
                addGlobalFacesMessage(FacesMessage.SEVERITY_INFO, "global.save", "global.success");
            } else {
                getLogger().warn("Could not update CompetenceGoal: '" + competenceGoal);
                addGlobalFacesMessage(FacesMessage.SEVERITY_ERROR, "global.save", "global.error");
            }
        } else {
            getLogger().warn("Could not update CompetenceGoal: '" + competenceGoal + "' since it was not stored in the view!");
            addGlobalFacesMessage(FacesMessage.SEVERITY_ERROR, "global.save", "global.error");
        }
    }
    ...
    public void removeCompetenceGoal(CompetenceGoal competenceGoal) {
        if (competenceGoals.contains(competenceGoal)) {
            if(competenceGoalBusiness.removeCompetenceGoal(competenceGoal)){
                addGlobalFacesMessage(FacesMessage.SEVERITY_INFO, "global.delete", "global.success");
                refreshCompetenceGoals();
            } else {
                getLogger().warn("Could not delete CompetenceGoal: '" + competenceGoal);
                addGlobalFacesMessage(FacesMessage.SEVERITY_ERROR, "global.delete", "global.error");
            }
        } else {
            getLogger().warn("Could not delete CompetenceGoal: '" + competenceGoal + "' since it was not stored in the view!");
            addGlobalFacesMessage(FacesMessage.SEVERITY_ERROR, "global.delete", "global.error");
        }
    }
...
//    public void loadCompetenceGoals() {
//        competenceGoalBusiness.addNewCompetenceGoalToCourse(
//                new CompetenceGoal(competence, (Course) course, 50, 1));
//    }
